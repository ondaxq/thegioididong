...
<form action="" method="post" enctype="multipart/form-data">
    ...
    <?php 
        if (isset($_POST['txtupdate'])) {
            ...
            $gia = $_POST['gia'];
            if (!empty($_FILES["hinhsp"]["name"])) {
                $urlhsp = basename($_FILES["hinhsp"]["name"]);
            } else {
                $hinh = $i_dt['hinhsp'];
            }
            if (!empty($_FILES["hinhts"]["name"])) {
            ...
            $update = $get_data->update_dt($ten, $hang, $hedieuhanh, $cpu, $gpu, $ram, $rom, $camsau, $camtruoc, $gia, $id, $urlhsp, $urlhts, $urlhqc );
            if ($update) {
                echo "<script>alert('Thành công');window.location='danhsachsanpham.php';</script>";
            } else {
